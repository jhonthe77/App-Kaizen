...
# Sección: Registro Diario
    ...
    st.header("Registro Diario de Hábitos (registro manual) ⏱️")
    ...
    for habit in habits:
        ...
        with col1:
            realizado = st.checkbox("✅ Hecho", key=f"chk_{habit}")
        ...
        with col3:
            if st.button("💾 Guardar", key=f"save_{habit}"):
                fila = {
                    "Fecha": datetime.now(BOGOTA).date().isoformat(),
                    "Hábito": habit,
                    "Realizado": "Sí" if realizado else "No",
                    "Hora Inicio": "",
                    "Hora Fin": "",
                    "Duración (min)": tiempo_manual if realizado else 0
                }
                ...
                st.success(f"📌 '{habit}' registrado correctamente")
...
# Sección: Estadísticas
    ...
    st.header("📊 Estadísticas de Tiempo por Hábito")
    ...
    filtro_tipo = st.radio("📅 Ver por:", ["Semana actual", "Semana específica", "Mes específico"], horizontal=True)

    if filtro_tipo == "Semana actual":
    ...
    elif filtro_tipo == "Semana específica":
        semanas_disp = df[["Año", "Semana"]].drop_duplicates().sort_values(["Año", "Semana"], ascending=[False, False])
        semanas_disp_str = [f"{row.Año} - Semana {row.Semana}" for row in semanas_disp.itertuples()]

        semana_sel = st.selectbox("📅 Selecciona una semana:", semanas_disp_str)

        año_sel, sem_sel = semana_sel.split(" - Semana ")
        año_sel = int(año_sel)
        sem_sel = int(sem_sel)
        ...
        meses_disp = sorted(df["Fecha"].dt.strftime("%Y-%m").unique(), reverse=True)
        mes_sel = st.selectbox("📅 Selecciona un mes:", meses_disp)
        df_filtrado = df[df["Fecha"].dt.strftime("%Y-%m") == mes_sel]
    ...
    df_filtrado["Día"] = df_filtrado["Fecha"].dt.strftime("%a")
    dias_orden = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
    df_filtrado["Día"] = pd.Categorical(df_filtrado["Día"], categories=dias_orden, ordered=True)

    # --- Tabs ---
    tab1, tab2 = st.tabs(["⏱️ Tiempo por hábito", "🔢 Actividades por día"])

    with tab1:
        resumen_duracion = df_filtrado.groupby(["Día", "Hábito"], observed=True)["Duración (min)"].sum().reset_index()
        ...
        fig_duracion = px.bar(
            ...
            text="Duración (min)",
            barmode="group",
            title="Duración total por hábito",
        ...
        max_val = df_filtrado["Duración (min)"].max()
        fig_duracion.update_yaxes(range=[0, max_val * 1.5])
        st.plotly_chart(fig_duracion, use_container_width=True)

    with tab2:
        resumen_actividades = df_filtrado.groupby(["Día", "Hábito"]).size().reset_index(name="Actividades")
        ...
        fig_actividades = px.bar(
            ...
            color="Hábito",
            barmode="group",
            title="Número de actividades registradas",
